...
if __name__ == '__main__':
    ...
    chunks = textwrap.wrap(alltext, width=3000)
    print(chunks)  # add this line to check the chunks variable
    result = ''
    for count, chunk in enumerate(chunks, 1):
        prompt = open_file('prompt.txt').replace('<<SUMMARY>>', chunk)
        prompt = prompt.encode(encoding='ASCII',errors='ignore').decode()
        print('\n\n\n', count, 'of', len(chunks), ' - chunk:', chunk, '- prompt:', prompt) # add this line
        summary = gpt3_completion(prompt)
        print('\n\n\n', count, 'of', len(chunks), ' - summary:', summary) # add this line
        result += summary + '\n\n'

    # Remove prompts from the result string
    result = re.sub(r'PROMPT:.*?RESPONSE:\n\n', '', result, flags=re.DOTALL)

    save_file(result, 'output_%s.txt' % time())